...
void intentar_establecer_conexion(int socket, char* tipo){
    stream_send_empty_buffer(socket, HANDSHAKE_interrupcion);
    uint8_t respuestaInterrupcion = stream_recv_header(socket);
    stream_recv_empty_buffer(respuestaInterrupcion);
