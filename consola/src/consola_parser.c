...
bool consola_parsear_instrucciones(t_buffer *buffer, const char *pathInstrucciones, t_log *consolaLogger) {
    ...
    char* parametro1 = malloc(sizeof(*parametro1));
    ...
    for (;;) {
        if (fscanf(archivoInstrucciones, "%s", instruccion)) {
            ...
            if (strcmp(instruccion, "F_READ") == 0) {
                if (fscanf(archivoInstrucciones, "%s %s %s", parametro1, parametro2, parametro3)) {
                ...
                if (fscanf(archivoInstrucciones, "%s %s", parametro1, parametro2)) {
                   consola_serializador_empaquetar_dos_parametros(buffer,INSTRUCCION_create_segment, parametro1, parametro2);
                   hayDosParametros = true;
                } else {
                    hayError = true;
            ...
            if (hayError) {
                ...
                exit(-1);
                      ...
                      log_info(consolaLogger, "Se empaqueta instruccion: %s con operandos %s , %s y %s", instruccion, parametro1, parametro2, parametro3);
            } else if (hayDosParametros) {
                      log_info(consolaLogger, "Se empaqueta instruccion: %s con operandos %s y %s", instruccion, parametro1, parametro2);
            } else if (hayUnParametro) { 
                      log_info(consolaLogger, "Se empaqueta instruccion: %s con operando %s", instruccion, parametro1);
            } 
            else {
